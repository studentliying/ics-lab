...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    switch(op)
	{
		...
		case A_AND: case A_XOR: case A_NONE: 
		    break;
	}
...
{
    bool_t doit = FALSE;
	bool_t z_f = GET_ZF(cc);
	bool_t s_f = GET_SF(cc);
	bool_t o_f = GET_OF(cc);
...
stat_t nexti(y64sim_t *sim)
{
    ...
    switch (icode) {
		...
		{
			byte_t reg_count = 0;
			if (!get_byte_val(sim->m, next_pc, &reg_count)) 
			{
           		err_print("PC = 0x%x, Invalid instruction address", sim->pc);
           		return STAT_ADR;
        	}
			regid_t rA = GET_REGA(reg_count);
			regid_t rB = GET_REGB(reg_count);
			long_t valA = get_reg_val(sim->r,rA);
      	...
      	case I_IRMOVQ: /* 3:0 F:regB imm */
		{
			...
			next_pc ++;
			long_t val = 0;
			if (!get_long_val(sim->m, next_pc, &val))
			{
           		err_print("PC = 0x%x, Invalid instruction address", sim->pc);
				return STAT_ADR;
			}
			set_reg_val(sim->r,rB,val);
			sim->pc += 10;
			break;
		}
      	case I_RMMOVQ: /* 4:0 regA:regB imm */
		{
			...
			next_pc ++;
			long_t val = 0;
			get_long_val(sim->m, next_pc, &val); 
			long_t valB = get_reg_val(sim->r, rB);
			long_t valm = valB + val;
			long_t valA = get_reg_val(sim->r, rA);
			set_long_val(sim->m, valm, valA);
			sim->pc += 10;
			break;
		}	
      	case I_MRMOVQ: /* 5:0 regB:regA imm */
        {
			...
			long_t valB = get_reg_val(sim->r,rB);
			long_t valm = valB + imm;
            long_t valt = 0;
			if (!get_long_val(sim->m, valm, &valt))
			{
           		err_print("PC = 0x%x, Invalid data address 0x%lx", sim->pc, valm);
           		return STAT_ADR;
			}	
			set_reg_val(sim->r, rA, valt);
         	sim->pc += 10;
         	break;
		 }
		...
		{
			long_t addr = 0;
			if (!get_long_val(sim->m, next_pc, &addr))
			{
           		err_print("PC = 0x%x, Invalid instruction address", sim->pc);
           		return STAT_ADR;
			}
      	...
      	case I_CALL: /* 8:x imm */
		{
			...
			long_t val_rsp;
			val_rsp = get_reg_val(sim->r, REG_RSP);
			set_reg_val(sim->r, REG_RSP, val_rsp - 8);
			if (!set_long_val(sim->m, val_rsp - 8, sim->pc + 9))
			{
            	err_print("PC = 0x%x, Invalid stack address 0x%lx", sim->pc, val_rsp - 8);
            	return STAT_ADR;
			}
			sim->pc = addr;
			break;
      	...
      	case I_RET: /* 9:0 */
		{
			long_t val_rsp = get_reg_val(sim->r,REG_RSP);
			long_t addr = 0;
        	set_reg_val(sim->r, REG_RSP, val_rsp + 8);
			if(!get_long_val(sim->m, val_rsp, &addr))
			{
				err_print("PC = 0x%x, Invalid stack address 0x%lx", sim->pc, val_rsp);
				return STAT_ADR;
			}
        	else sim->pc = addr;
       	 	break; 
      	...
      	case I_PUSHQ: /* A:0 regA:F */
		{
        	...
        	long_t val_rsp = get_reg_val(sim->r, REG_RSP);
        	set_reg_val(sim->r, REG_RSP, val_rsp-8);
			if (!set_long_val(sim->m, val_rsp-8, valA))
			{
				err_print("PC = 0x%x, Invalid stack address 0x%lx", sim->pc,val_rsp - 8);
				return STAT_ADR;
			}	
      	...
      	case I_POPQ: /* B:0 regA:F */
		{
			...
			long_t val_rsp = get_reg_val(sim->r, REG_RSP);
			long_t val = 0;
			get_long_val(sim->m, val_rsp, &val);
			set_reg_val(sim->r, REG_RSP, val_rsp+8);
        	set_reg_val(sim->r, rA, val);
        	sim->pc += 2;
        	break;
		}
